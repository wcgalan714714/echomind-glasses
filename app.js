...
function renderInsights() {
    ...
    getInsights().forEach(insight => {
        ...
        // Tap to start conversation on THIS card only
        card.onclick = () => startConversationOnCard(card, insight);
        panel.appendChild(card);
    });
}

// Conversation happens only inside the tapped card (other cards stay)
function startConversationOnCard(cardElement, insight) {
    const originalHTML = cardElement.innerHTML;

    cardElement.innerHTML = `
        <strong>${insight.title}</strong><br>
        <p style="margin: 12px 0; font-size: 17px;">${insight.content}</p>
        <p style="margin-bottom: 12px; opacity: 0.9;">Was this useful?</p>
        
        <div style="display:flex; flex-direction:column; gap:8px;">
            <button class="focusable" onclick="handleYesNoMaybe(this, '${insight.id}', true, '${originalHTML}')">Yes</button>
            <button class="focusable" onclick="handleYesNoMaybe(this, '${insight.id}', false, '${originalHTML}')">No</button>
            <button class="focusable" onclick="handleYesNoMaybe(this, '${insight.id}', null, '${originalHTML}')">Maybe</button>
        </div>
    `;
}

function handleYesNoMaybe(element, insightId, wasUseful, originalHTML) {
    const card = element.closest('.insight-card');

    // Save feedback
    if (!feedback[insightId]) feedback[insightId] = 0;
    if (wasUseful === true) feedback[insightId] += 1;
    if (wasUseful === false) feedback[insightId] -= 1;
    ...
    if (wasUseful === true) {
        ...
        card.innerHTML = `
            ...
            <div style="display:flex; flex-direction:column; gap:8px; margin-top:8px;">
                <button class="focusable" onclick="endConversation(this, true, '${originalHTML}')">Yes</button>
                <button class="focusable" onclick="endConversation(this, false, '${originalHTML}')">No</button>
            </div>
        `;
    } else {
        endConversation(element, false, originalHTML);
    }
}

function endConversation(element, positive, originalHTML) {
    const card = element.closest('.insight-card');

    if (positive) {
        card.innerHTML = `
            <strong>Got it.</strong><br>
            <p style="margin-top:10px;">I’ll show you more like this.</p>
        `;
    } else {
        card.innerHTML = `<strong>Thanks for the feedback.</strong>`;
    }

    // Restore the card after a short moment
    setTimeout(() => {
        card.innerHTML = originalHTML;
        // Re-attach click event
        const insight = getInsights().find(i => i.id === card.dataset.id);
        if (insight) {
            card.onclick = () => startConversationOnCard(card, insight);
        }
    }, 1400);
...
// All other functions stay the same (captureNote, voice, history, etc.)
